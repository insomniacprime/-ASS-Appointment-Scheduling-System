 <?php 
    ...
    $s = "SELECT * FROM asa.appointments WHERE id = '$id'";
    $res = $con->query($s);
                            ...
                            <div class="panel panel-default">
                                <div class="panel-body">
                                    <form role="form" method="post" action="updateappointment.php">
                                            ...
                                            <?php
                                                if($result->num_rows >0)
                                                {
                                                    while($row = $result->fetch_assoc())
                                                    {
                                                        if($headerid == $row['id'])
                                                        {
                                                            echo "<option value='". $row['id'] ."' selected = 'selected'>". $row['fname']. "</option>";    
                                                        }
                                                        else
                                                        {
                                        ...
                                        <div class="form-group">
                                            <label>Purpose</label>
                                            <input class="form-control" name="purpose" value="<?php echo $purpose ?>" >
                                            
                                        </div>
                                        <div class="form-group">
                                            <label>Date [yyyy-mm-dd]</label>
                                            <input class="form-control" name="appdate" value="<?php echo $appdate ?>" />
                                        </div>
                                        <div class="form-group">
                                            <label>Time [hh:mm AM/PM]</label>
                                            <input class="form-control" name="apptime" value="<?php echo $apptime ?>" />\
                                            <input class="form-control" type="hidden" name="appid" value="<?php echo $appid ?>" />
                                        </div>
